...
def read_txt_file(filename):
    ...
    with open(filename) as f:
        ...
        init_quick_sort(unsorted_list, counter)

        # Quick Select calculations (time, median, dist)
        init_quick_select(unsorted_list, counter)

def init_quick_sort(unsorted_list, counter):
    """
    initialises the run for quick sort
    :param unsorted_list: unsorted list
    :param counter: the number of buildings
    :return: None
    """
    ...
    print("\tSum of distances:", sum_of_dist(median,
                                             sorted_set_qsort, counter))

def init_quick_select(unsorted_list, counter):
    """
    initialises the run for quick select
    :param unsorted_list: unsorted list
    :param counter:  the number of buildings
    :return: None
    """
    start = time.perf_counter()
    qselect = run_quick_select(unsorted_list,
                               len(unsorted_list) // 2)
    time_elapsed = time.perf_counter() - start
    print("Using QuickSelect to find optimal location")
    print("\tElapsed time:", time_elapsed, "seconds")
    print("\tOptimal food truck location :", qselect)
    print("\tSum of distances:", sum_of_dist(qselect,
...
def run_quick_sort(unsorted_list):
    ...
    if len(unsorted_list) == 0:
        ...
        result = run_quick_sort(less_list) +\
                 equal_list + run_quick_sort(greater_list)
    return result
...
def sum_of_dist(median, list, count):
    """
    Calculates the sum of distances from
            median to different points
